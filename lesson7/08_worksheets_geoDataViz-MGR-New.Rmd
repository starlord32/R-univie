...
(@) Plot 2: *This graphs shows Churches that were build in different timeperiods.* (but also do not forget to add all necessary information into the map/cartogram).
...
The `europop` package has information on population of european cities, which we can now map. There are two main tables:  `europop` with the historical populations of European cities, and `city_coords`âwith the latitudes and longitudes of those cities. With this new data you now should be able to map European cities. Your task is as follows:
