...
> dataset, y axis and x axis incl. axis names

(@) What is the relationship between each row in the dataset and each glyph in the plot?

> how many confessions did lead to a convertion
...
```{r}
ggplot(paulist_missions, aes(x = converts, y = duration_days, color = confessions))+
...
ggplot(paulist_missions, aes(x = converts, y = duration_days, color = confessions))+
  ...
  labs(title = "Paulist missions",
       x = "Convertions",
       y = "inquisition duration in days",
       color = "Desity of confessions")
```
...
```{r}
ggplot(paulists_by_year, aes(y = confessions, x = year))+
  geom_line()
```

(@) Do you see any issues with the current graph? Please, explain.

> The main issue with line graphs is that they do not reflect missing data which coud be the case between 1865 and 1871.
...
```{r}
library(ggplot2)
ggplot(paulists_by_year, aes(x = year, color = missions))+
  geom_line(aes(y = confessions))+
  geom_line(aes(y = converts))+
  labs(title = "Paulist missions",
       x = "Time",
       y = "Conversions and Confessions")
```
...
> It counts/aggregats the number of observations at each location.
...
```{r}
ggplot(mn_missions_with_labels, aes(x = confessions, y = converts))+
  geom_point()+
...
ggplot(early_colleges, aes(x = established, y = state))+
  geom_count()+
  geom_text_repel(aes(label = college))+
  labs(title="Colleges in US", 
       y="Amount of colleges", 
       x="State")

```

> This plot has two purposes. The first one is to find out which colleges was founded first in which state and how much time passed till the next one was established. The second purpose is to see when most colleges were established and which states established several colleges in the same period of time. The text labels are not the best option here but allow to find out the name of the college that was established first. 
...
ggplot(gapminder, aes(x = year, y = lifeExp, color = continent))+
  ...
  labs(title = "World's life expecantcy over time",
       x = "Time",
       y = "Lifeexpectancy in Years",
       color = "Continents")
```

> This plot provides information about how the life expentancy changed over time on different continents. While the plot has problems showing the dots for Oceania the averge line provides better information about which continent has the highest life expectancy. I choose geom_count to visualize the amount of countries being part of a continent if there is a spontatneous increase in life expenctancy at some time. One circle evolved around 1962 were life expectancy had been rising in several countries. The geom_count also visualizes that espeicially Africa has a great range that counts from about 30 to around 70 depending on the country. Other continents like Oceania seem to be quite homogeneous. The geom_smooth also indicates how Asia could catch up to other from the 1990s onwards. 
...
ggplot(gapminder, aes(x = gdpPercap, y = lifeExp, color = continent))+
  geom_point()+
  geom_smooth(method="lm", se=F)+
  labs(title = "World's life expectancy and GDP",
       x = "GDP in USD",
       y = "Lifeexpectancy in Years",
       color = "Continents")
```

> A comparison of GDP per capita and country polulation seems to somehow interessting. High GDP does not necessarily mean that the country has a high life expectancy as in asian countries over 60000 and 90000. The trend however, tends to a growing life expenctancy wiht higher GDP. I used geom_point to receive the density of countries of a specific ratio. geom_smooth provides good information with countries are above or below the average of their continent. Labeling the dots with text did not proivde any usabel results. 
