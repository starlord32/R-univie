...
library(tidyverse)
library(historydata)
load("C:/Users/Daniel/Documents/R_course/R-univie/lession5/methodists.rda")
```
...
methodists %>%
  filter(year >= 1786) %>%
  group_by(year) %>%
  summarise(avg_white = mean(members_white), avg_black = mean(members_black), avg_indian = mean(members_indian), avg_total = mean(members_total))
```
