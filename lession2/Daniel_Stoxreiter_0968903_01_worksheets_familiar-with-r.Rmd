...
```{r}
paste("I am", "sittinging", "and", "typing", "stuff")
```

(@) Now explain in a sentence what happened.

> WIth the paste() function the five character strings were added to one and printed.
...
> The first one is TRUE since 2 is smaller 3. The second is FALSE since 2 is not greater than 3. The third one is FALSE since 2 does not euqal 3.
...
> NA stands for not a number and if 2 adds NA than the output is NA because the calculations does not result in a number but NA since the calculation does not work out. 
...
> The variable is "computer_name". The assignment operator is "<-". The assignment is a character string "HALP 9000".
...
> A numeric value 2 was aissigned to x. A numeric value 5 was assigned to y. Two calculations with x(2) * y(5) = 10 and x(2) + 9 = 11.
...
> It muliplies all 5 values times 2. 
...
> Because the length of the left vector is greater than the length of the right one. 
...
> Show all book prices that are greater or equal to the median of book_prices.
...
> Because some elements or NA and not a value.
...
The observations are the rows and the variables the columns. 
...
> It comes from the "dplyr" package.
...
```{r}
  con <- head(paulist_missions$converts, 20)
...
```{r}
  boxplot(formula = confessions ~ converts, data = paulist_missions)
```
...
> The plot has the title "Confessions versus conversions". It will display the number of confessions and conversions.
