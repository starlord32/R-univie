...
```{r}
  even_numbers2 <- seq(2, 26, by= 2)
  letters[even_numbers2]
```
...
names(song_rankings) <- c("song1", "song2", "song3", "song4", "song5")
song_rankings["song3"]
```
...
> It has 5705 rows and 5 columns. It contains integer, character and a factor vector. 
...
```{r}
  earliest_date <- head(sort(naval_promotions$date, decreasing=FALSE))
  latest_date <- tail(sort(naval_promotions$date, decreasing=TRUE))
  earliest_date[1]
  latest_date[1]
```
...
>  No, the $ sign accesses specifc variables (columns) and returns everything inside if no further optional is declared. The "[" "[[" signs access specific vectors of a e.g. dataset. 
...
```{r}
  my_list <- list(
  title = c("WWI", "WWII"),
  yearStart = c(1914, 1939), 
  yearEnd = c(1918, 1945),
  participants1 = c("Germany", "Austria", "France", "Great Britain", "USA", "Japan", "Russia", "Others")
)
my_list$title[1]
my_list$yearStart[1]
my_list$yearEnd[1]
...
```{r}
 captains <- naval_promotions[naval_promotions$rank == "captain", ]
 head(captains, 15)
```
...
library(dplyr)
  library("lubridate") 
  navalDates <- naval_promotions$date
  navalDateYear <- ymd(navalDates)
  year_1800 <- year(navalDateYear)
  head(year_1800 > 1799 & year_1800 < 1900)
```
